...
func Example_createCliqueGenesis() {
	a := []AddressBalance{
		{
			Address: "7df9a875a174b3bc565e6424a0050ebc1b2d1d82",
			Balance: "300000",
	...
	genesis, err := CreateCliqueGenesis(chainID, period, epoch, difficulty, gasLimit, signerAddr, a)
	if err != nil {
		fmt.Println(err)
	}
...
func Example_ethashGenersis() {
	...
	difficulty := uint64(1)
	gasLimit := uint64(8000000)
	genesis, err := CreateEthashGenesis(chainID, difficulty, gasLimit, a)
	if err != nil {
		fmt.Println(err)
	}
	fmt.Println(string(genesis))
